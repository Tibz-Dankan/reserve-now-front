...
import { updateRoomBeds } from "../API";
...
export const UpdateRoomBeds = () => {
  ...
  const { isLoading, data, mutate } = useMutation({
    ...
    onSuccess: (data) => {
      ...
      dispatch(updateEditRoom(data.data.room));
      setSelectedBeds([]);
    },
    onError: (error) => {
    ...
    <Fragment>
      <div className="relative">
        <div className="border-b-[1px] border-gray-opacity mb-1 pb-2">
          <h1 className="text-xl font-semibold text-gray-dark-4">Beds</h1>
        </div>
        ...
        <div className="w-full overflow-x-hidden ">
          <div className="py-2">
            <span className="text-base font-light">
              Existing bed(s) (available in the room)
            </span>
          </div>
        ...
        <div className="pt-2">
          <div className="py-2">
            <span className="text-base font-light">
              Select new beds for the room
            </span>
          </div>
          <div
            className="grid gap-2 grid-cols-2 md:grid-cols-3 lg:grid-cols-4
           w-full h-[25vh] overflow-x-hidden"
          >
            {selectedBeds.map((bed, index) => {
              return (
                <div className="" key={index}>
                  <LabelTag className="bg-primary">
                    <div className="flex items-center justify-center gap-x-2">
                      <div className="relative group">
                        ...
                        <span
                          className="absolute top-4 left-[88px] invisible opacity-0 w-
            ...
            <form
              onSubmit={(event) => updateBedsHandler(event)}
              className="h-[18vh]"
            >
              <div className="inline-block w-full text-start">
                <select
                  ...
                  first-letter:focus:bg-gray-200 transition-all outline-none p-2 rounded-3xl
              ...
              <div
                className="border-t-[1px] border-gray-opacity p-4 pl-0 absolute 
             bottom-0 right-0 left-0 z-50 bg-gray-light-1 rounded-bl-lg rounded-br-lg"
              >
                ...
                {isLoading && <Loader label="Editing" className="w-36" />}
              </div>
            </form>
          </div>
        </div>
